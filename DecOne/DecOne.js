...
const data = fs.readFileSync('./DecOne.txt', 'UTF-8');
  ...
  const maxElf = Object.keys(elves).filter(
     (key) => elves[key] === maxCals
  );

  return ['Elf(s) with most calories: ', maxElf, 'max calorie amount: ', maxCals];
...
const calCount2 = (arrayOfData) => {
    ...
    const maxElves = Object.keys(elves).filter(
        (key) => elves[key] === topThree[0] || elves[key] === topThree[1] || elves[key] === topThree[2]
    );
  
    return ['Elves with the most calories: ', maxElves, 'max calorie amount: ', maxCals];
